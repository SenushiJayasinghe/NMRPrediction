...
# ---- MLP / training config ----
ARCHITECTURE = (512, 256, 128)   # hidden layer sizes
N_FOLDS      = 10
EPOCHS       = 150
BATCH_SIZE   = 16
LR           = 1e-3
...
DERIVATIVE_LOSS_LAMBDA = 0.5        # NEW -- weight of the point-to-point derivative loss term

# ---- Auxiliary multiplicity-classification head (NEW) ----
...
for fold, (tr_idx, val_idx) in enumerate(kf.split(X_train_pool), start=1):
    ...
    if val_mse[-1] < best_val_mse_overall:
        ...
        print(f"  New best model saved (Val MSE {best_val_mse_overall:.6f})")
    print()
...
final_model.eval()
with torch.no_grad():
    test_preds_t, test_aux_logits = final_model(X_test_t)
    test_preds = test_preds_t.cpu().numpy()
    test_aux_pred_class = test_aux_logits.argmax(dim=1)
    test_aux_acc = (test_aux_pred_class == Y_test_mult_labels_t).float().mean().item()
...
plt.suptitle(f'Visual Inspection — MLP {ARCHITECTURE} (GroupNorm) '
...
summary = {
    'Model_Type': ['Flat MLP + GroupNorm + Auxiliary Multiplicity Head'],
    ...
    'Test_R2': [test_r2],
    'Test_MAE': [test_mae],
    'Test_Aux_PointAcc': [test_aux_acc],
...
print("\n" + "="*60)
print(f"ALL DONE — MLP {ARCHITECTURE} (GroupNorm, aux head)")
