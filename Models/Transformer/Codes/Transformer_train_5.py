...
INTEGRATION AS AN AGREEMENT METRIC (carried over from the CNN script,
same rationale and implementation):
...
# signature (a repeating pattern once per bin -- see script history/chat
# for the diagnosis). Shrunk down here to a size that can actually be
...
# ---- Stability fixes (NOT in the source code -- see docstring) ----
...
# ---- Loss: faithful default is plain MSE (WEIGHT_ALPHA=0, DERIVATIVE_LOSS_LAMBDA=0) ----
# Set WEIGHT_ALPHA > 0 and/or DERIVATIVE_LOSS_LAMBDA > 0 to bring in this
# project's peak-weighted / derivative-loss improvements instead of the
# paper's plain MSE -- left OFF by default to keep this a faithful
# adaptation of the validated published recipe.
WEIGHT_ALPHA = 0.0
DERIVATIVE_LOSS_LAMBDA = 0.0
