...
# MODIFICATION: Added nn.Softplus() activation layer at the end of the 
# Transformer to prevent negative values in the predicted spectrum.
# ============================================================================
...
# ---- Original architecture hyperparameters ----
D_MODEL = 128 # changed from 512 -> 128
N_HEADS = 4 # changed from 8 -> 4
N_ENCODER_LAYERS = 3 # changed from 6 -> 3
DIM_FEEDFORWARD = 512 # changed from 2048 -> 512
DROPOUT = 0.1
PREFERRED_INPUT_DIM = 32 # changed from 1000 -> 32

# ---- Original training hyperparameters ----
EPOCHS = 300 # changed from 500 -> 300
BATCH_SIZE = 32
PATIENCE = 25
N_FOLDS = 10
N_TEST = 52 # size of the held-out test set
...
# Keep 60 MHz and 90 MHz as two SEPARATE channels (stacked, not concatenated end-to-end)
...
def detect_peaks(spectrum, height=PEAK_FIND_HEIGHT, prominence=PEAK_FIND_PROMINENCE, distance=PEAK_FIND_DISTANCE):
...
# ============================================================================
# MODEL — Original Transformer with added Softplus
# ============================================================================

class Transformer(nn.Module):
    def __init__(self, input_dim, d_model, nhead, num_encoder_layers, dim_feedforward, dropout=0.1, in_channels=2):
        ...
        # Softplus activation function to enforce non-negative spectrum intensities
    ...
    def forward(self, x):
        ...
        # Apply Softplus before returning output
        x = self.output_act(x)
        return x
...
if INPUT_DIM != PREFERRED_INPUT_DIM:
    print(f"NOTE: {n_points} points isn't evenly divisible by the original bin size {PREFERRED_INPUT_DIM}; using {INPUT_DIM} instead.")
...
def train_fold(X_tr, Y_tr, X_val, Y_val, epochs, batch_size, input_dim, patience=PATIENCE):
    ...
    for epoch in range(epochs):
        ...
        if (epoch + 1) % 10 == 0 or epoch == 0:
            print(f" Epoch {epoch+1}/{epochs} — Train Loss {train_loss:.6f} Val Loss {val_loss:.6f} Train R2 {train_r2:.4f} Val R2 {val_r2:.4f}")
...
print(f"\nStarting {N_FOLDS}-fold CV — original architecture (d_model={D_MODEL}, heads={N_HEADS}, layers={N_ENCODER_LAYERS})\n")
fold_results = []
...
for fold, (tr_idx, val_idx) in enumerate(kf.split(X_train_pool), start=1):
    ...
    print(f" Final -- Train Loss {tr_loss[-1]:.6f} R2 {tr_r2[-1]:.4f} | Val Loss {val_loss[-1]:.6f} R2 {val_r2[-1]:.4f} | Best epoch {best_epoch}\n")
...
for i, r in enumerate(fold_results):
    ...
    ax.set_title(f"Fold {r['Fold']} — Train {r['Final_Train_Loss']:.5f} / Val {r['Final_Val_Loss']:.5f}", fontsize=9)
    ax.set_xlabel('Epoch', fontsize=8)
    ax.set_ylabel('Loss (MSE)', fontsize=8)
    ax.legend(fontsize=7)
plt.suptitle(f'Per-Fold Train/Val Loss — Original Transformer (d={D_MODEL}, {N_ENCODER_LAYERS}L, {N_HEADS}H)', fontsize=12, fontweight='bold')
plt.tight_layout()
...
ax1.fill_between(x_axis, train_loss_i.mean(0)-train_loss_i.std(0), train_loss_i.mean(0)+train_loss_i.std(0), color='royalblue', alpha=0.2)
ax1.plot(x_axis, val_loss_i.mean(0), color='darkorange', label='Mean Val Loss')
ax1.fill_between(x_axis, val_loss_i.mean(0)-val_loss_i.std(0), val_loss_i.mean(0)+val_loss_i.std(0), color='darkorange', alpha=0.2)
...
ax2.fill_between(x_axis, train_r2_i.mean(0)-train_r2_i.std(0), train_r2_i.mean(0)+train_r2_i.std(0), color='forestgreen', alpha=0.2)
ax2.plot(x_axis, val_r2_i.mean(0), color='crimson', label='Mean Val R2')
ax2.fill_between(x_axis, val_r2_i.mean(0)-val_r2_i.std(0), val_r2_i.mean(0)+val_r2_i.std(0), color='crimson', alpha=0.2)
ax2.set_xlabel('Epoch'); ax2.set_ylabel('R2')
ax2.set_title('Mean R2 — Original Transformer')
ax2.legend()

plt.suptitle(f'{N_FOLDS}-Fold CV Performance — Original Transformer (d={D_MODEL}, {N_ENCODER_LAYERS}L, {N_HEADS}H)', fontsize=14, fontweight='bold')
plt.tight_layout()
...
plt.title(f'Predicted vs Simulated 500 MHz Intensity — Test Set\nOriginal Transformer (d={D_MODEL}, {N_ENCODER_LAYERS}L, {N_HEADS}H) | MAE = {test_mae:.6f} | R2 = {test_r2:.4f}', fontsize=12)
...
for tau in POSITION_TOLERANCES_PPM:
    ...
    if len(valid) > 0 and valid['N_Matched'].sum() > 0:
        pooled_mean_error = float(np.average(valid['Mean_Position_Error_ppm'], weights=valid['N_Matched']))
    else:
...
arch_summary = {
    'Model_Type': ['Transformer (Johnson & Tipirneni-Sajja 2024, unmodified hyperparameters; 60/90 MHz as separate channels concatenated per bin) + Softplus'],
    'D_Model': [D_MODEL],
    'N_Heads': [N_HEADS],
...
print("\n" + "="*60)
print(f"ALL DONE — Original Transformer with Softplus, d_model={D_MODEL}, heads={N_HEADS}, layers={N_ENCODER_LAYERS}, input_dim={INPUT_DIM}")
print(f" Final single model trained for {FINAL_EPOCHS} epochs")
