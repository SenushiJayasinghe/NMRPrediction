...
17, 18. 
Sections 7 (model), 14 (single-vs-ensemble test eval), 19
(integration), 20 (architecture-summary CSV) were deliberately left out.
There is NO ensembling anywhere in this script (the original model doesn't
use one).

ONE unavoidable adaptation:
the original model takes ONE low-field spectrum in and reconstructs a
same-length spectrum out (both axes are the same 46,000-point grid — it's
just doing 100 MHz -> 400 MHz on the same points). My data has TWO
low-field spectra (60 + 90 MHz). Rather than touch the Transformer class:
  1. INPUT:  the 60 MHz and 90 MHz spectra (each length N) are concatenated
             end-to-end into one sequence of length 2N before it enters the
             (unchanged) model.
  2. TARGET: for the loss, the true 500 MHz spectrum (length N) is pasted
             twice to length 2N, so both halves of the model's output are
             each supervised to look like the true spectrum.
  3. OUTPUT: the model reconstructs a length-2N vector (same as input, as
             always). At prediction time this is split into its two
             N-length halves and averaged elementwise into one final
             predicted spectrum.
No line inside the Transformer class changes because of this — it always
just sees "a sequence in, the same-length sequence out."
"""
...
# ---- Required adaptation (agreed): concatenate the two low-field inputs
# end-to-end into one sequence. The model class itself is never touched —
...
print(f"X_all shape (60+90 concatenated): {X_all.shape}   Y_all shape: {Y_all.shape}")
...
# ============================================================================
# MODEL — copied verbatim from the original notebook, unchanged.
# (Deliberately not "Section 7")
# ============================================================================
...
# the (doubled, 60+90-concatenated) sequence length ----
def _resolve_input_dim(seq_len, preferred=PREFERRED_INPUT_DIM):
...
INPUT_DIM = _resolve_input_dim(SEQ_LEN)
if INPUT_DIM != PREFERRED_INPUT_DIM:
    print(f"NOTE: sequence length {SEQ_LEN} isn't evenly divisible by the original "
          f"bin size {PREFERRED_INPUT_DIM}; using {INPUT_DIM} instead.")
print(f"Bin size (input_dim): {INPUT_DIM}  ->  {SEQ_LEN // INPUT_DIM} bins per (60+90) sequence")
...
#
# Because the model's output is the doubled (60+90) sequence length, the
# target is duplicated to match it for the loss, and predict() below
# averages the two output halves back down to a single spectrum.
# ============================================================================

def predict(model, X_batch):
    """Run the model and average its two (60-half, 90-half) output halves
    down to a single n_points-long prediction."""
    out = model(X_batch)                      # (batch, 2*n_points)
    half = out.shape[1] // 2
    return (out[:, :half] + out[:, half:]) / 2.0
...
def run_epoch(model, loader, optimizer, train=True):
    ...
    with context:
        for xb, yb in loader:
            ...
            if train:
                optimizer.zero_grad()
            out = model(xb)                              # (batch, 2*n_points)
            target_doubled = torch.cat([yb, yb], dim=1)   # supervise both halves
            loss = criterion(out, target_doubled)
            if train:
                loss.backward()
                optimizer.step()
            total_loss += loss.item() * xb.size(0)
            preds = predict(model, xb)
...
with torch.no_grad():
    test_preds = predict(final_model, X_test_t).cpu().numpy()
