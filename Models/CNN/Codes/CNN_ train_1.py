...
Pipeline is the same as the MLP version:
  ...
  4. 10-fold CV (~405 train / ~45 val per fold).
  5. Retrain final model on the full 450-compound pool.
...
class NMRCNN(nn.Module):
    ...
    Output: (batch, n_points)      -- predicted 500 MHz intensity
...
for fold, (tr_idx, val_idx) in enumerate(kf.split(X_train_pool), start=1):
    ...
    model, tr_mse, val_mse, tr_r2, val_r2 = train_fold(
    ...
    print(f"  Final -- Train MSE {tr_mse[-1]:.6f} R2 {tr_r2[-1]:.4f} | "
          f"Val MSE {val_mse[-1]:.6f} R2 {val_r2[-1]:.4f}")
...
for epoch in range(EPOCHS):
    ...
    if (epoch + 1) % 10 == 0 or epoch == 0:
        print(f"  Epoch {epoch+1}/{EPOCHS} — Train MSE {train_mse:.6f}  Train R2 {train_r2:.4f}")
