...
# AUX_LOSS_WEIGHT reduced from 0.3 -> 0.05, and class weights below switched
# from raw inverse-frequency to sqrt-scaled + capped: raw inverse-frequency
# weighting reached ~419x for the rarest class (Singlet), which -- even
# multiplied by a 0.3 loss weight -- let the classification loss's gradient
# magnitude swamp the regression signal. This showed up directly as CV
# Train R2 collapsing from ~0.77 (no aux head) to ~0.43 (aux head, raw
# weights) despite the regression loss itself being unchanged. Same fix
# already applied in the Transformer/Wave-U-Net scripts after the same
# diagnosis there.
...
# Class weights for the auxiliary classification loss -- sqrt-scaled inverse
# frequency (not raw inverse frequency), balanced so the dominant class
# (usually 'None'/background) doesn't make the auxiliary head collapse to
# always predicting it. Raw inverse-frequency weighting hit ~419x for the
# rarest class here, which let the classification loss's gradient magnitude
# dominate and destabilise the regression task (see AUX_LOSS_WEIGHT comment
# above) -- sqrt-scaling still up-weights rare classes but far less
# aggressively.
